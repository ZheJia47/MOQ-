...
function renderTable (targetH, Level, rows, reportRange, iReportDate, myChart2, period, fun, labelText, columnsObj, defectiveGroup, defectiveTableTimes, loading, topName) {
  var targetDetail = {} // 保存原始請求作為產生上級資料的參考
  var levelNum = 0
  Object.keys(labelText).forEach(del => {
    Vue.delete(labelText, del)
  })
  Object.keys(myChart2).forEach(del => {
    Vue.delete(myChart2, del)
  })
