  ...
  <section class="content">
   <div class="row inline bg-light-blue-7 full-width glossy">
    ...
    {{dateFrom}} ~ {{dateTo}}</span>
      ...
      <pie-chart :label="pieLabelTotal" :data="pieDataTotal" :color="producePieColor" :options="myChart2Options"></pie-chart>
    </div>
    <div v-show="group" class="row justify-center q-mt-xl">
      <!--<p> <span>{{displayDepartment}}</span>-->
      <p> <span>{{topName}}</span>
      <q-table
        :data="rowsGroup"
        :columns="columns"
      ...
      <pie-chart :label="pieLabelGroup" :data="pieGroup" :color="producePieColor" :options="myChart2Options"></pie-chart>
    </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      pieGroup: [],
      pieLabelGroup: [],
      pieData: {}, // [9.4, 9.5, 37.5, 8],
      myChartLabel: {}, // ['6A', '6B', '6C', '8F'],
  ...
  watch: {
    ...
    group () { // 不重載，圓餅圖不會出現，但若一開始有顯示，隱藏在出現則沒事，反之會異常
      this.updateRenderTable() // this.constructTable()
    }
  },
  mounted () {
  ...
  methods: {
    updateRenderTable () {
      ...
      console.log('to')
      console.log(to)
      renderTable(this.columns, this.rows, this.zoneS, from, to, this.labelText, this.pieData, this.myChartLabel, this.$q.loading, this.rowsTotal, this.pieDataTotal, this.pieLabelTotal, this.orgArr, this.displayDepartment, this.rowsGroup, this.pieGroup, this.pieLabelGroup, this.topName)
    },
    print () {
      window.print()
    },
...
function renderTable (columns, rows, zoneS, from, to, labelText, pieData, myChartLabel, loading, rowsTotal, pieDataTotal, pieLabelTotal, orgArr, displayDepartment, rowsGroup, pieGroup, pieLabelGroup, topName) {
  /* var Level = [{
  ...
  // console.log('進入')
  var machineIDArr = recursiveMakeMachineArr(zoneS)
  var failTotalMs = {}
  var unitArr = []
  var Level = []
  ...
  var zoneSValues = Object.values(tempZoneS)
  // console.log(22222222222222222222222)
  // console.log(zoneSValues[0])
  Object.keys(myChartLabel).forEach(del => {
    Vue.delete(myChartLabel, del)
  })
  Object.keys(pieData).forEach(del => {
    Vue.delete(pieData, del)
  })
  Object.keys(rows).forEach(del => {
    Vue.delete(rows, del)
  })
  Object.keys(labelText).forEach(del => {
    Vue.delete(labelText, del)
  })
  // var baseIndex = 0
  zoneSValues[0].forEach((base, idx) => {
    /* labelText[baseIndex] = []
    ...
    // console.log(base)
    labelText[idx] = []
    var subKeys = (typeof base === 'object') ? Object.keys(base)[0] : base
    // console.log(subKeys)
    var subLevel = Level.find(elm => {
      return elm._id === subKeys
    })
    switch (subLevel.level) {
      case 1:
        labelText[idx].splice(0, 0, subLevel._id)
        break
      case 2:
        labelText[idx].splice(0, 0, subLevel._id + subLevel.levelName)
        break
      default:
        labelText[idx].splice(0, 0, subLevel._id)
        break
    ...
    // 問後台取得
    levelObj[subLevel._id] = recursiveFindMachineID(Level, subLevel._id, -1)
    unitArr.splice(unitArr.length, 0, subLevel._id)
  })
  // ----------------取得機台編號
  ...
  $.get('/api/getAbnormalTable/' + machineIDArr + '/' + from + '/' + to + '/0', {}, (results) => {
      ...
      cntDataKeys.forEach(cntDataKey => {
        ...
        if (cntDataKey === 'totalErr') {
          if (totalFailHr > 0) {
            ...
            groupCnt[cntDataKey] += totalFailHr
          } else {
            percentage = 0
          ...
          if (totalFailHr > 0) {
            groupCnt[cntDataKey] += cntData[cntDataKey]
    ...
    rowsGroup.splice(0, rowsGroup.length)
    pieGroup.splice(0, pieGroup.length)
    pieLabelGroup.splice(0, pieLabelGroup.length)
    cntDataKeys.forEach(cntDataKey => {
      /* var totalHour = 0
      rowsKeys.forEach(rowsKey => {
        var temp = rows[rowsKey].find(elm => {
          return elm.reason === errStr[rowsKey]
        })
        // console.log('ooo')
        // console.log(temp)
        totalHour += 1
      }) */
      rowsGroup.splice(rowsGroup.length, 0, {
        reason: errStr[cntDataKey],
        totalHour: formatSecond(groupCnt[cntDataKey] / 1000),
        percentage: (groupCnt[cntDataKey] > 0) ? `${(Math.round(groupCnt[cntDataKey] / groupCnt['totalErr'] * 1000) / 10)}%` : '0%'
      })
      if (cntDataKey !== 'totalErr') {
        pieGroup.splice(pieGroup.length, 0, groupCnt[cntDataKey])
        pieLabelGroup.splice(pieLabelGroup.length, 0, errStr[cntDataKey])
      }
    })
    // 新增
    rowsTotal.splice(0, rowsTotal.length)
    pieDataTotal.splice(0, pieDataTotal.length)
    pieLabelTotal.splice(0, pieLabelTotal.length)
    ...
    unitArr.forEach(mch => {
      if (failTotalMs.hasOwnProperty(mch)) {
        calcuTotMs += failTotalMs[mch]
      }
    })
    if (zoneSValues.length >= 0) {
      // console.log(zoneSValues[0])
      // console.log(zoneSValues[0][0])
      // console.log(Object.keys(zoneSValues[0]))
      Object.keys(zoneSValues[0]).forEach(elm => {
        // var transferObj = {}
        // transferObj[elm] = zoneSValues[0][elm]
        var rowTotalUnit = (typeof zoneSValues[0][elm] === 'object') ? Object.keys(zoneSValues[0][elm])[0] : zoneSValues[0][elm]
        var machineArr = recursiveMakeMachineArr(zoneSValues[0][elm])
        var calculusMs = failTotalMs[rowTotalUnit]
        ...
        rowsTotal.splice(rowsTotal.length, 0, {
          ...
          percentage: (calcuTotMs > 0) ? `${(Math.round(calculusMs / calcuTotMs * 1000) / 10)}%` : '0%',
          count: machineArr.length
        })
        pieDataTotal.splice(pieDataTotal.length, 0, calculusMs)
        pieLabelTotal.splice(pieLabelTotal.length, 0, rowTotalUnit)
      })
    }
    rowsTotal.splice(rowsTotal.length, 0, {
      unit: '合計',
      totalHour: formatSecond(calcuTotMs / 1000),
      percentage: '100%',
      count: machineIDArr.length
    })
    loading.hide()
    // console.log('--------------')
    // console.log(rows)
