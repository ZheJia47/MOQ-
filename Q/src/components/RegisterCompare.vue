...
<section class="content">
  ...
  <!-- dropdown content -->
      ...
      <q-btn outline glossy label='複製到機台' icon="icon-svg58" @click="coptToMachineDB()"/>
      <q-btn outline glossy label='刪除指定資料' icon="icon-svg58" @click="deleteToMachineDB()"/>
      <q-btn outline glossy label='全部複製' icon="icon-svg58" @click="copyAllToMachineDB()"/>
      <q-btn outline glossy label='全部刪除' icon="icon-svg58" @click="deleteAllToMachineDB()"/>
      </div>
    ...
    <!--<q-list>-->
     ...
     <!--<q-item>-->
      <div class="row inline justify-end q-mb-xs full-width">
       ...
       <q-btn outline size="16px" text-color="black" text-bold label="全選" @click="selecetedAll"/>
       </div>
...
<!--           <q-select
 ...
 -->
        ...
        <q-td slot="body-cell-check" slot-scope="cell">
          ...
          <q-checkbox v-model="cell.row.check"/>
        </q-td>
...
export default {
  ...
  sockets: {
    coptToMachineDBSuccess: function (val) {
      ...
      this.status = '複製生產紀錄成功'
      setTimeout(() => {
        this.status = null
      }, 5000)
    },
    deleteToMachineDBSuccess: function (val) {
      renderTable(this.rows, this.machineID, this.selectedDate, this.selectedDateTo, this.orderNumberList, this.noteListArr, this.rowArray, this.$q.loading)
      this.status = '刪除生產紀錄成功'
      setTimeout(() => {
        this.status = null
      }, 5000)
    },
  ...
  methods: {
    deleteToMachineDB () {
      var selectedRows = this.rows.filter(row => {
        return row.check === true
      }).map(row => row._id)
      this.$socket.emit('deleteToMachineDB', {key: this.machineID, value: selectedRows})
    },
    coptToMachineDB () {
      var selectedRows = this.rows.filter(row => {
        return row.check === true
      }).map(row => row._id)
      this.$socket.emit('coptToMachineDB', {key: this.machineID, value: selectedRows})
    },
    copyAllToMachineDB () {
      this.$socket.emit('copyAllToMachineDB', {key: this.machineID, from: this.selectedDate, to: this.selectedDateTo})
    },
    deleteAllToMachineDB () {
      this.$socket.emit('deleteAllToMachineDB', {key: this.machineID, from: this.selectedDate, to: this.selectedDateTo})
    },
...
function renderTable (rows, machineID, iReportDate, selectedDateTo, orderNumberList, noteListArr, rowArray, loading) {
  ...
  $.get('/api/getRegisterCompare/' + machineID + '/' + reportDate + '/' + reportDateTo, {}, (results) => {
    if (results.type !== true) {
      alert(`response is not correct, err: ${results.data}`)
      ...
      records.forEach(record => {
        ...
        var t = moment.duration(moment(record['lastTrigTime'] | '00:00').diff(moment(record['trigTime'] | '00:00')))
        console.log('00000000000000000000000000')
        console.log(record['lastTrigTime'])
        console.log(moment(record['lastTrigTime']).format('MM-DD HH:mm:ss') + '.' + Math.round(Number(moment(record['lastTrigTime']).format('SSS')) / 100))
        console.log((Math.round(Number(moment(record['lastTrigTime']).format('SSS')) / 100) >= 10) ? (moment(record['lastTrigTime']).add(1, 'seconds').format('MM-DD HH:mm:ss') + '.0') : (moment(record['lastTrigTime']).format('MM-DD HH:mm:ss') + '.' + Math.round(Number(moment(record['lastTrigTime']).format('SSS')) / 100)))
        // var t = moment.duration(moment(record['lastTrigTime']) - moment(record['trigTime']))        var H
        var H
        var m
        var s
        var ms
