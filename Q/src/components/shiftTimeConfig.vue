  ...
  <section class="content">
   <div class="row inline bg-light-blue-7 full-width glossy">
    ...
    <div class="col-6 q-pt-md q-mt-md">
       <div class="row justify-end q-pt-sm q-pb-xs q-pr-xl text-black">
     ...
     <q-btn v-show="editShift" outline glossy label='取消' @click="cancelShiftSetting(), editShift=false"/>
      ...
      <q-tr slot="body" slot-scope="props" :props="props">
        ...
        <q-td v-bind:key="col.name" v-else-if="props.row['newData'] === true && col.name === 'shiftName'">
          <q-input v-model="props.row[col.name + '0']" type="text"/>
        </q-td>
...
export default {
  ...
  mounted () {
    $.get('/api/getShiftClock', {}, (results) => {
      if (results.type !== true) {
        ...
        this.shift = results.data[0]
        this.initDoc = results.data[1]
        // })
        // console.log('旗標')
    ...
    rows1 () {
      this.rows1.forEach(row1 => {
        ...
        this.columns1.forEach((column, index) => {
          if (index === 2) {
            from = row1[column.field]
          } else if (index === 4) {
            to = row1[column.field]
          }
        })
        if (to < from) {
          this.$set(row1, this.columns1[3].field, this.columns1[3].label)
        } else {
          this.$set(row1, this.columns1[3].field, '')
        }
      })
      console.log('rows1')
  ...
  methods: {
    ...
    writeClockSetting () {
      ...
      this.$socket.emit('writeClockSetting', this.initDoc)
      // console.log(this.initDoc)
    },
    newShift () {
    ...
    writeShiftSetting () {
      ...
      this.rows1.forEach(row1 => {
        ...
        if (row1.newData === true) {
          ...
          this.shift[row1['shiftName']] = {'from': Number(row1['from0']), 'to': Number(row1['to0'])}
        }
        if ((_.invert(this.initDoc))[row1['to']] < (_.invert(this.initDoc))[row1['from']]) {
          this.$set(row1, this.columns1[3].field, this.columns1[3].label)
        } else {
          this.$set(row1, this.columns1[3].field, '')
        }
      ...
      var sendObj = {}
      var shiftKeys = Object.keys(this.shift).sort((a, b) => {
        return (a === '_id') ? 1 : (b === '_id') ? -1 : (Number(this.shift[a].from) > Number(this.shift[b].from)) ? 1 : -1
      })
      var idx = 1
      shiftKeys.forEach(elm => {
        // console.log(elm)
        if (elm === '_id') {
          sendObj[elm] = this.shift[elm]
          return
        }
        sendObj[elm] = {}
        sendObj[elm]['from'] = Number(this.shift[elm].from)
        sendObj[elm]['to'] = Number(this.shift[elm].to)
        sendObj[elm]['key'] = idx.toString()
        idx++
      })
      // console.log('this.shift: ')
      // console.log(sendObj)
      this.$socket.emit('writeShiftSetting', sendObj)
    },
    cancelShiftSetting () {
      this.rows1.forEach((row1, ind, arr) => {
        Vue.set(row1, 'select', false)
