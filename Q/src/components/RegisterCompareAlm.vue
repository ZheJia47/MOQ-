...
<section class="content">
     <div class="row inline bg-light-blue-7 full-width glossy">
      <div class="col-12 q-pl-xl text-white">
         <h4>警報資料比對 - Alarm Data Compare</h4>
      </div>
      </div>
  ...
  <!-- dropdown content -->
      ...
      <q-btn outline glossy label='刪除全部警報' icon="icon-svg58" @click="deleteAlmRecord()"/>
      </div>
    ...
    <!--<q-list>-->
     ...
     <div class="col q-pt-xs q-pb-xs">
      <h1 v-if="status === null">射出機警報資料比較
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      自{{from}}~{{to}}</h1>
      <h1 v-else>射出機生產資料比較 ({{status}})
         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      自{{from}}~{{to}}</h1>
   </div>
      <!--</q-item>-->
...
export default {
  ...
  sockets: {
    clrAlmRecordAllSuccess: function (val) {
    ...
    clrAlmRecordAllErr: function (val) {
      renderTable(this.rows, this.machineID, this.selectedDate, this.selectedDateTo, this.orderNumberList, this.noteListArr, this.rowArray, this.$q.loading)
      this.status = '清除記錄失敗'
      setTimeout(() => {
        this.status = null
  ...
  methods: {
    deleteAlmRecord () {
      this.$socket.emit('clrAlmRecordAll', {
        machineID: this.machineID,
        from: moment(this.selectedDate).format('YYYY-MM-DD 08:00:00'),
        to: moment(this.selectedDateTo).format('YYYY-MM-DD 08:00:00')
      })
    },
    deleteToMachineDB () {
...
function renderTable (rows, machineID, iReportDate, selectedDateTo, orderNumberList, noteListArr, rowArray, loading) {
  ...
  $.get('/api/getRegisterCompareAlm/' + machineID + '/' + reportDate + '/' + reportDateTo, {}, (results) => {
    if (results.type !== true) {
      alert(`response is not correct, err: ${results.data}`)
      ...
      // console.log(records)
      records.sort((a, b) => {
        return moment(a.trigTime).isBefore(b.trigTime) ? -1 : 1
      })
