    ...
    <!--<q-list>-->
     ...
     <!--<q-item>-->
      <div class="row inline justify-end q-mb-xs full-width">
       ...
       <q-btn outline glossy label='清除派單號碼' @click="clrMulOrderNumber()"/>
       </div>
...
function renderTable (rows, machineID, iReportDate, selectedDateTo, orderNumberList, noteListArr, rowArray) {
  ...
  $.get('/api/getMachineRecords/' + machineID + '/' + reportDate + '/' + reportDateTo, {}, (results) => {
    ...
    if (results.type !== true) {
      ...
      records.forEach(record => {
        ...
        rows.splice(rows.length, 0, {
          ...
          min: ((record['moldCount'] || 0) > 0) ? ((Math.round((record['min'] || 0) * 10) / 10) === 0) ? 0 : (Math.round((record['min'] || 0) * 10) / 10).toFixed(1) : 0,
