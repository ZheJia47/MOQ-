...
<section class="content">
  ...
  <!-- dropdown content -->
      ...
      <q-btn outline glossy label='EXCEL匯出' icon="icon-svg57" @click="ExcelExport(rows, columns, machineID + '資料登錄')"/>
    ...
    <!--<q-list>-->
     ...
     <!--<q-item>-->
      <div class="row inline justify-end q-mb-xs full-width">
       ...
       <q-btn outline size="16px" text-color="black" text-bold label="載入" @click="reloadTable"/>
        ...
        <q-btn outline size="16px" text-color="black" text-bold label='清除派單號碼' @click="clrMulOrderNumber()"/>
       </div>
     ...
     {{(checkArr.length > 0) ? checkArr[0].row.trigTime : ''}}&nbsp;{{(checkArr.length > 0) ? checkArr[checkArr.length - 1].row.lastTrigTime : ''}}
      <!--</q-item>-->
<div class="row justify-center full-width q-mt-xs">
 ...
 <!--  <q-scroll-area style="width: 400px; max-width: 85vw; height: 470px; max-height: 70vh;" class="bg-grey-3 round-borders shadow-2">-->
      ...
      >
        ...
        <q-td slot="body-cell-orderNumber" slot-scope="cell" :style="cell.row.orn ? 'color: red;' : 'color: black;'" >
<!--           <q-select
            v-if="edit"
            filter
            toggle
 ...
 -->
        ...
        <q-td slot="body-cell-moldNum" slot-scope="cell" :style="cell.row.mdn ? 'color: green;' : 'color: black;'">
          <q-select
            v-if="(cell.row.check|edit)"
            filter
            toggle
...
export default {
  ...
  sockets: {
    clrMulOrderNumberSuccess: function (val) {
      renderTable(this.rows, this.machineID, this.selectedDate, this.selectedDateTo, this.orderNumberList, this.noteListArr, this.rowArray)
      this.status = '派單號碼清除成功'
      setTimeout(() => {
        this.status = null
      }, 5000)
    },
    clrMulOrderNumberFail: function (val) {
      renderTable(this.rows, this.machineID, this.selectedDate, this.selectedDateTo, this.orderNumberList, this.noteListArr, this.rowArray)
      this.status = '派單號碼清除失敗'
      setTimeout(() => {
        this.status = null
      }, 5000)
    },
  ...
  methods: {
    clrMulOrderNumber () {
      if (this.checkArr.length > 0) {
        this.checkArr.sort((a, b) => {
          return moment(a.row.trigTimeFullType).isBefore(b.row.trigTimeFullType) ? -1 : 1
        })
        var msg = {}
        msg['machineID'] = this.machineID
        msg['trigTime'] = this.checkArr[0].row.trigTimeFullType
        msg['lastTrigTime'] = this.checkArr[this.checkArr.length - 1].row.lastTrigTimeFullType
        this.$socket.emit('clrMulOrderNumber', msg)
        // console.log(msg)
    ...
    checkFunc (cell) {
      ...
      if (cell.row.check) {
        if (this.checkArr.map(x => x.row._id).indexOf(cell.row._id) > -1) {
          this.checkArr.splice(this.checkArr.map(x => x.row._id).indexOf(cell.row._id), 1)
          // console.log('AAAAAAAAAAAAAA')
        }
        // console.log(this.checkArr.map(x => x.row._id).indexOf(cell.row._id))
        // console.log('BBBBBBBBBBBBB')
        /* this.checkArr.push(cell)
        var msg = {}
        msg['_id'] = cell.row._id
        msg['machineID'] = cell.row.machineID
        msg['moldCount'] = cell.row.moldCount
        this.$socket.emit('setMoldCount', msg) */
      } else {
        ...
        if (results.type !== true) {
          ...
          results.data.forEach((result) => {
            this.orderNumberList.push({
              label: result, // result['派單號碼'],
              value: result // result['派單號碼']
            })
          })
          $.get('/api/getNoteSelectedItem', {}, (docs) => {
            if (docs.type !== true) {
...
function renderTable (rows, machineID, iReportDate, selectedDateTo, orderNumberList, noteListArr, rowArray) {
  ...
  $.get('/api/getMachineRecordsWA/' + machineID + '/' + reportDate + '/' + reportDateTo, {}, (results) => {
    if (results.type !== true) {
      alert(`response is not correct, err: ${results.data}`)
      ...
      records.forEach(record => {
        ...
        if (record.machineStatusType === '警報-通信') {
          return
        ...
        rows.splice(rows.length, 0, {
          ...
          trigTime: (Math.round(Number(moment(record['trigTime']).format('SSS')) / 100) >= 10) ? (moment(record['trigTime']).add(1, 'seconds').format('MM-DD HH:mm:ss') + '.0') : (moment(record['trigTime']).format('MM-DD HH:mm:ss') + '.' + Math.round(Number(moment(record['trigTime']).format('SSS')) / 100)),
          ...
          average: ((record['moldCount'] || 0) === 0) ? 0 : Math.round(moment.duration(moment(record['lastTrigTime']) - moment(record['trigTime'])).asMilliseconds() / Number(record['moldCount']) / 100) / 10, // record['average'] ? record['average'] : 0,
      ...
      rows.forEach((row, idx, arr) => {
        if (idx > 0) {
          if (temp === '') {
            temp = row.orderNumber
          } else {
            if (row.orderNumber !== '') {
              if (temp !== row.orderNumber) {
                temp = row.orderNumber
                Vue.set(row, 'orn', true)
              }
            }
          }
          if (moldNum === '') {
            ...
            if (row.moldNum !== '') {
              if (moldNum !== row.moldNum) {
                moldNum = row.moldNum
                Vue.set(row, 'mdn', true)
              }
            }
          }
        } else {
